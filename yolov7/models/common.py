...
class SP(nn.Module):
    ...
    def forward(self, x):
        """Forward pass through strided max pooling layer."""
        return self.m(x)
...
class ReOrg(nn.Module):
    ...
    def forward(self, x):  # x(b,c,w,h) -> y(b,4c,w/2,h/2)
        """Forward pass through reorganization layer."""
...
class Concat(nn.Module):
    ...
    def forward(self, x):
        """Forward pass through concatenation."""
        return torch.cat(x, self.d)


class Chuncat(nn.Module):
    """Chunk and concatenate tensors."""
    ...
    def forward(self, x):
        """Forward pass through chunk and concatenate."""
        x1 = []
        x2 = []
        for xi in x:
...
class Shortcut(nn.Module):
    ...
    def forward(self, x):
        """Forward pass through shortcut connection."""
        return x[0]+x[1]


class Foldcut(nn.Module):
    """Fold and cut operation."""
    ...
    def forward(self, x):
        """Forward pass through fold and cut."""
...
class Conv(nn.Module):
    ...
    def fuseforward(self, x):
        """Forward pass through fused convolution."""
...
class RobustConv(nn.Module):
    ...
    def forward(self, x):
        """Forward pass through robust convolution."""
...
class RobustConv2(nn.Module):
    ...
    def __init__(self, c1, c2, k=7, s=4, p=None, g=1, act=True,
        ...
        self.conv_deconv = nn.ConvTranspose2d(
            in_channels=c1, out_channels=c2, kernel_size=s, stride=s,
            padding=0, bias=True, dilation=1, groups=1
        )
        self.gamma = nn.Parameter(layer_scale_init_value * torch.ones(c2)) if layer_scale_init_value > 0 else None

    def forward(self, x):
        """Forward pass through robust convolution 2."""
...
class DownC(nn.Module):
    ...
    def forward(self, x):
        """Forward pass through downsample CSP."""
...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        """Forward pass through darknet bottleneck."""
...
class BottleneckCSPA(nn.Module):
    ...
    def forward(self, x):
        """Forward pass through BottleneckCSPA."""
...
class BottleneckCSPB(nn.Module):
    ...
    def forward(self, x):
        """Forward pass through BottleneckCSPB."""
        x1 = self.cv1(x)
        y1 = self.m(x1)
        y2 = self.cv2(x1)
        return self.cv3(torch.cat((y1, y2), dim=1))
...
class BottleneckCSPC(nn.Module):
    ...
    def forward(self, x):
        """Forward pass through BottleneckCSPC."""
...
class ImplicitA(nn.Module):
    ...
    def forward(self, x):
        """Forward pass through implicit addition."""
...
class ImplicitM(nn.Module):
    ...
    def forward(self, x):
        """Forward pass through implicit multiplication."""
        return self.implicit * x
...
class RepConv(nn.Module):
    ...
    def get_equivalent_kernel_bias(self):
        """Get equivalent kernel and bias."""
...
class Focus(nn.Module):
    ...
    def forward(self, x):  # x(b,c,w,h) -> y(b,4c,w/2,h/2)
        """Forward pass through focus layer."""
...
class Contract(nn.Module):
    ...
    def forward(self, x):
        """Forward pass through contract layer."""
...
class Expand(nn.Module):
    ...
    def forward(self, x):
        """Forward pass through expand layer."""
...
class NMS(nn.Module):
    ...
    def forward(self, x):
        """Forward pass through NMS module."""
...
class autoShape(nn.Module):
    ...
    def forward(self, imgs, size=640, augment=False, profile=False):
        """Forward pass through autoShape wrapper."""
        ...
        for i, im in enumerate(imgs):
            ...
            if isinstance(im, str):  # filename or uri
                im, f = np.asarray(Image.open(
                    requests.get(im, stream=True, timeout=30).raw
                    if im.startswith('http') else im
                )), im
...
class Detections:
    ...
    def display(self, pprint=False, show=False, save=False, render=False, save_dir=''):
        ...
        for i, (img, pred) in enumerate(zip(self.imgs, self.pred)):
            msg = f'image {i + 1}/{len(self.pred)}: {img.shape[0]}x{img.shape[1]} '
            if pred is not None:
                for c in pred[:, -1].unique():
                    n = (pred[:, -1] == c).sum()  # detections per class
                    msg += f"{n} {self.names[int(c)]}{'s' * (n > 1)}, "  # add to string
...
class ConvBN(nn.Module):
    ...
    def switch_to_deploy(self):
        ...
        conv = nn.Conv2d(
            in_channels=self.conv.in_channels, out_channels=self.conv.out_channels,
...
class OREPA_3x3_RepConv(nn.Module):
    ...
    def weight_gen(self):
        ...
        if self.groups > 1:
            ...
            weight_rbr_1x1_kxk = (
                torch.einsum('gti,gothw->goihw', weight_rbr_1x1_kxk_conv1,
        ...
        weight = (
            weight_rbr_origin + weight_rbr_avg + weight_rbr_1x1_kxk +
    ...
    def dwsc2full(self, weight_dw, weight_pw, groups):
        """Convert depthwise separable conv to full conv."""
...
class RepConv_OREPA(nn.Module):
    ...
    def __init__(self, c1, c2, k=3, s=1, padding=1, dilation=1, groups=1,
        ...
        if deploy:
            self.rbr_reparam = nn.Conv2d(
                ...
                kernel_size=k, stride=s, padding=padding, dilation=dilation,
    ...
    def get_custom_L2(self):
        ...
        eq_kernel = k3[:, :, 1:2, 1:2] * t3 + k1 * t1  # The equivalent resultant central point.
...
class WindowAttention_v2(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        logit_scale = torch.clamp(
            self.logit_scale, max=torch.log(torch.tensor(1. / 0.01))
        ).exp()
